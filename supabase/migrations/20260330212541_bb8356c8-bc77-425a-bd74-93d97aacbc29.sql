...
BEGIN
  ...
  -- Real Income (Paid) — EXCLUDE transfers to avoid double-counting
  ...
  -- Real Expense (Paid) — EXCLUDE transfers
  ...
  -- Previous Period Income — EXCLUDE transfers
  ...
  -- Previous Period Expense — EXCLUDE transfers
  ...
  -- Total services (by scheduled_date, non-cancelled)
  ...
  -- Completed services — use scheduled_date for consistency with total
  ...
  -- Revenue by type (completed, by scheduled_date for consistency)
  ...
  WHERE organization_id = _org_id AND status = 'cancelled'
    AND (scheduled_date >= (_sd::text || ' 00:00:00')::timestamptz AND scheduled_date <= (_ed::text || ' 23:59:59')::timestamptz)
    AND deleted_at IS NULL AND (_is_demo OR is_demo_data = false);
