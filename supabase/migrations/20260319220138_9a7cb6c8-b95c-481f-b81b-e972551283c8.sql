-- Migrate legacy conversion_status values to the new 4-stage funnel
UPDATE public.whatsapp_contacts
SET conversion_status = 'em_atendimento'
WHERE conversion_status IN ('aguardando_cliente', 'orcamento_enviado');

UPDATE public.whatsapp_contacts
SET conversion_status = 'concluido'
WHERE conversion_status IN ('finalizado', 'recorrencia');

-- Update the RPC to use new funnel stages
CREATE OR REPLACE FUNCTION public.get_whatsapp_report_stats(_org_id uuid, _channel_id uuid DEFAULT NULL::uuid, _date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, _date_to timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  counts AS (
    SELECT
      ...
      -- New 4-stage funnel counts
      COUNT(*) FILTER (WHERE conversion_status = 'concluido')::int AS converted,
      COUNT(*) FILTER (WHERE conversion_status IN ('em_atendimento', 'agendado', 'concluido'))::int AS attended,
  ...
  by_assignee AS (
    SELECT assigned_to, COUNT(*)::int AS cnt,
           COUNT(*) FILTER (WHERE conversion_status = 'concluido')::int AS converted_count
    FROM filtered_contacts
