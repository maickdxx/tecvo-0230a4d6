...
-- Fix get_dashboard_stats: correct period filtering and ticket médio calculation
...
BEGIN
  ...
  -- Income: paid income transactions in period (by payment_date)
  ...
  -- Forecasted revenue: scheduled OR in_progress services in period (not yet completed)
  ...
  -- Total services in period (by scheduled_date for consistency)
  SELECT COUNT(*) INTO _total_services FROM services
  WHERE organization_id = _org_id
    AND scheduled_date >= _start_date AND scheduled_date <= (_end_date::date + 1)::text
    AND deleted_at IS NULL AND document_type IS DISTINCT FROM 'quote'
  ...
  -- Pending services in period (filtered by scheduled_date)
  ...
  -- Cancelled services in period (by scheduled_date)
