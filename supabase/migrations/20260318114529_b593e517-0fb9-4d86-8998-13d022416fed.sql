...
-- Validate entry type sequence on INSERT
-- Blocks: clock_out/break_start/break_end without prior clock_in on same day
-- Blocks: consecutive same-type entries (e.g., two break_starts in a row)
...
BEGIN
  ...
  -- clock_out -> (nothing, already blocked by prevent_duplicate_clock_in for clock_in)
...
CREATE TRIGGER trg_validate_entry_sequence
  BEFORE INSERT ON public.time_clock_entries
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_entry_sequence();
