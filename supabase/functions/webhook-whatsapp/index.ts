...
/**
 * fetchOrgContext — delegates to shared module with debug logging
 */
async function fetchOrgContext(supabase: any, organizationId: string) {
  const ctx = await sharedFetchOrgContext(supabase, organizationId);
  console.log(
...
/**
 * buildSystemPrompt — delegates to shared module, adds WhatsApp channel suffix
 */
function buildSystemPrompt(ctx: any) {
  let prompt = sharedBuildSystemPrompt(ctx);

  // WhatsApp-specific channel instructions
  prompt += `\n\nCANAL: WhatsApp. SEM markdown complexo. Texto simples + emojis. Use *negrito* (estilo WhatsApp).
...
- Quando send_service_pdf retornar resultado começando com "SILENT_PDF_SENT:" ou "SILENT_PDF_SENT_SELF:", significa que o PDF já foi enviado com sucesso. Confirme ao usuário de forma natural.
- Após criar OS/orçamento e o usuário pedir o PDF, use send_service_pdf imediatamente.
- Após criar cliente com sucesso, continue AUTOMATICAMENTE criando a OS ou orçamento pendente. NÃO peça para o usuário repetir os dados.`;
