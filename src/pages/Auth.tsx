...
export default function Auth() {
  ...
  const trackLeadAfterConfirmedSignup = (email: string) => {
    const normalizedEmail = email.trim().toLowerCase();
    if (!normalizedEmail) return;

    const storageKey = `tecvo_fb_lead_${normalizedEmail}`;
      ...
      <div className="min-h-screen flex items-center justify-center bg-background px-4">
        <div className="w-full max-w-sm space-y-6 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">Enviamos um código de 6 dígitos para</p>
      ...
      <div className="w-full max-w-[440px] space-y-5">
        ...
        {invite && (
          <div className="p-4 rounded-lg bg-primary/5 border border-primary/20 text-center">
            ...
            <p className="font-semibold text-lg text-foreground">{invite.organizations?.name || "Organização"}</p>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-background px-3 text-muted-foreground">{mode === "signup" ? "ou cadastre com email" : "ou entre com email"}</span>
          </div>
        </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Link to="/redefinir-senha" className="text-xs text-primary hover:underline">
                  Esqueci minha senha
                </Link>
              </div>
              <Input
          ...
          <form onSubmit={handleSignup} className="space-y-3.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                required
                readOnly={!!invite}
                className={`h-12 rounded-xl text-base ${invite ? "bg-muted" : ""}`}
              />
