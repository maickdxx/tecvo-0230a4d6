...
function AudioPlayer({ src, isMe }: { src: string; isMe: boolean }) {
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      // Fallback: some browsers only resolve duration after play starts
      if (!metadataLoaded) updateDuration();
    };
  ...
  return (
    <div className="flex items-center gap-2.5 min-w-[200px]">
      ...
      <Mic className={cn("h-3.5 w-3.5 shrink-0", isMe ? "text-primary-foreground/40" : "text-muted-foreground/40")} />
    </div>
  );
}
    ...
    <>
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          <div
            className={cn(
              "rounded-2xl shadow-sm overflow-hidden transition-opacity",
              isMe
