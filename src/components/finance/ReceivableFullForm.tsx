...
interface ReceivableFullFormProps {
...
export function ReceivableFullForm({ onSubmit, isSubmitting, onCancel, initialData }: ReceivableFullFormProps) {
  const tz = useOrgTimezone();
  const { groupedIncomeCategories } = useTransactionCategories("income");
  ...
  const [isReceived, setIsReceived] = useState(initialData?.status === "paid");
  ...
  const handleSubmit = async () => {
    ...
    if (isEditing) {
      await onSubmit({
        ...
        payment_date: isReceived ? (compensationDate || getTodayInTz(tz)) : undefined,
        payment_method: isReceived ? (paymentMethod || undefined) : undefined,
        recurrence: installmentsEnabled ? recurrenceType : undefined,
        client_id: clientId || undefined,
        financial_account_id: financialAccountId,
        notes: notes || undefined,
        compensation_date: isReceived && compensationDate ? compensationDate : undefined,
      } as any);
      return;
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      await onSubmit({
        ...
        status: isReceived ? "paid" : "pending",
        payment_date: isReceived ? (compensationDate || formatDateObjInTz(parcelDue, tz) || getTodayInTz(tz)) : undefined,
    ...
    <div className="space-y-6">
      <Tabs defaultValue="financial" className="w-full">
        ...
        <TabsContent value="financial" className="space-y-6 mt-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input
                  id="description"
                  placeholder="Ex: Recebimento de serviço de manutenção"
                  value={description}
              ...
              <div>
                ...
                <Select value={category} onValueChange={setCategory}>
                  ...
                  <SelectContent>
                    {groupedIncomeCategories.map((group) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Status do Recebimento</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <Switch
                ...
                />
                <Label>Pagamento já recebido?</Label>
              </div>
              {isReceived && (
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Data real em que o dinheiro entrou na conta bancária.
                    </p>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    O lançamento será salvo como <strong>recebido</strong>.
                  </p>
                </div>
              )}
              {!isReceived && (
                <p className="text-sm text-muted-foreground">
                  ...
                  {isEditing && initialData?.status === "paid" && (
                    <span className="block mt-1 text-xs text-destructive">
                      Ao desmarcar, o saldo bancário será revertido automaticamente.
                    </span>
                  )}
                </p>
              )}
      ...
      <div className="flex justify-between items-center pt-4 border-t">
        ...
        <Button onClick={handleSubmit} disabled={isSubmitting || !description.trim() || gross <= 0 || !financialAccountId || !category} className="gap-2">
          ...
          {isSubmitting ? "Salvando..." : isEditing ? "Salvar alterações" : "Salvar conta a receber"}
        </Button>
      </div>
    </div>
  );
