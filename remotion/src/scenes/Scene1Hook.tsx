...
export const Scene1Hook = () => {
  ...
  const barWidth = interpolate(barScale, [0, 1], [0, 80]);
  ...
  const lineWidth = interpolate(frame, [55, 75], [0, 200], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  const badgeSpring = spring({ frame: frame - 65, fps, config: { damping: 12 } });

  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center", padding: 80 }}>
      <div style={{ position: "absolute", top: 60, left: 80, width: barWidth, height: 3, background: "#2547D0", borderRadius: 2 }} />
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ width: lineWidth, height: 3, background: "linear-gradient(90deg, #2547D0, rgba(37,71,208,0.2))", margin: "28px auto", borderRadius: 2 }} />

        <p style={{ fontFamily: "sans-serif", fontSize: 22, color: "rgba(232,234,240,0.6)", fontWeight: 400, opacity: subOpacity, transform: `translateY(${subY}px)`, letterSpacing: 0.5 }}>
      ...
      >
        <div style={{ width: 36, height: 36, borderRadius: 9, background: "#2547D0", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ color: "white", fontWeight: 900, fontSize: 18, fontFamily: "sans-serif" }}>T</span>
        </div>
        <span style={{ color: "#E8EAF0", fontFamily: "sans-serif", fontWeight: 700, fontSize: 20 }}>tecvo</span>
      </div>
    </AbsoluteFill>
  );
};
